...
foreach ( glob( $dir . '/*.php' ) ?: array() as $file ) {
	...
	$class = 'WcCoreTables\Schemas\\' . $base;
...
$report    = FlagReadiness::score( 'WooCommerce', $supported, $declared );

printf( "\n== WooCommerce behavioral readiness (flags) ==\n" );
...
file_put_contents( $out . '/woocommerce.json', Badge::toJson( $report ) );
printf( "  wrote %s/woocommerce.json\n\n", $out );
